...
Get CORE results data from their sftp server, copy new files to bslcene
Write all CORE results to one db table

20150615: Removed unnecessary functions to create views for separate sealion and bedrock views
20150615: Removed unnecessary function to create csv file of CORE results for sealion and bedrock results
...
localFolder = "/data/polarview/core/monitoring_results/"
...
def writeCOREtoDBase(csvFile):

    fileName = csvFile.split('/')[3]
    print "Processing CORE csv file in %s and writing data to database public.core_results_all" % (fileName)
        ...
        for row in bergreader:
            ...
            if row[2] == ' ':
                ...
                try:
                    ...
                    # writing to database
                    query = "INSERT INTO public.core_results_all (targetnum, filename, sourcerow, sourcecol, area, maj_axis, min_axis, wrlnLen, detconf, detclass, acqtime, latlong, lat, long, prodtype, origfilename) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, ST_Force_2d(ST_GeomFromText(%s, 4326)), %s, %s, %s, %s)"
#                     print query
        ...
        conn.commit()           
        print "CORE results written to database table core_results_all"
        curs.close()
        conn.close()
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description=("Checks CORE ftp server for new directories. "
  ...
  parser.add_argument("-d", help="directory name of CORE results to write to database.", action="store", dest='dirName')
  ...
  try:
    ...
    i = 0
    for m in re.finditer('downloadCORE_results', output ):
      i += 1
    if i > 1:
      print 'Process downloadCORE_results.py already running -> exit'
    else:
      sys.exit(main())
  except Exception, e:
